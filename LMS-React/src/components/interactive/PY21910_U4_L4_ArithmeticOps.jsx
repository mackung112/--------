      ...
      <div className="flex flex-col min-h-[450px]">
        <div className="flex flex-col lg:flex-row flex-1">
          ...
          <div className="flex-1 p-6 border-b lg:border-b-0 lg:border-r border-slate-200">
            <h4 className="font-base text-sm font-medium tracking-wide uppercase text-slate-500 mb-4">เลือกการคำนวณ</h4>
            <div className="grid grid-cols-2 gap-3">
              {operations.map((op, i) => (
                ...
                >
                  ...
                  <div className="text-xs text-slate-500">{op.expr}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-4 space-y-1 flex-1" ref={consoleRef}>
            {history.map((line, idx) => (
              <div key={idx} className="leading-relaxed">
                {line.type === 'command' && (
                  <div className="text-slate-300"><span className="text-green-400 mr-2">{">>>"}</span>{line.text}</div>
                )}

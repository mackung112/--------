...
export default function PY21910_U4_L6_ComparisonOps() {
  ...
  return (
    <div className="w-full my-12">
      ...
      <div className="flex justify-center gap-4 mb-6">
        <div className="text-center"><label className="text-xs text-gray-700">a</label><input type="number" value={a} onChange={e => setA(+e.target.value)} className="block w-20 text-center text-xl font-bold border-2 border-indigo-300 rounded-xl p-2" /></div>
        <div className="text-center"><label className="text-xs text-gray-700">b</label><input type="number" value={b} onChange={e => setB(+e.target.value)} className="block w-20 text-center text-xl font-bold border-2 border-indigo-300 rounded-xl p-2" /></div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {ops.map((op, i) => (
          <div key={i} className={`rounded-2xl p-4 text-center border-2 transition-all ${op.result ? 'bg-green-50 border-green-300' : 'bg-red-50 border-red-300'}`}>
            ...
            <div className="text-xs text-gray-700 mt-1">{op.name}</div>
          </div>
        ))}
      </div>
    </div>

...
- **ขั้นตอนการสร้างหน้าใหม่ (New Page Creation Workflow)**:
  1. **สร้างไฟล์ Component**: สร้างไฟล์ใน `\Teach\LMS-React\src\components\interactive\` (เช่น `pyUnitX_X_Name.jsx`)
  ...
  3. **เชื่อมโยงข้อมูล**: นำชื่อไฟล์ Component ไปใส่ในรูปแบบ `[Marker]` ในฟิลด์ `content` ของไฟล์ Database (เช่น `pyCourse.js`) พร้อมตรวจสอบให้แน่ใจว่าฟิลด์อื่นๆ ทั้ง 5 ฟิลด์ระบุข้อมูลครบถ้วน
  4. **การเขียนทับทันที (Overwrite Policy)**: หากผู้ใช้สั่งให้สร้างหน้าใหม่ หรือระบุใหม่อีกครั้ง ให้สร้างขึ้นมาใหม่ทั้งหมดโดยไม่ต้องกังวลเรื่องโค้ดหรือข้อมูลที่มีอยู่เดิมในไฟล์ และเขียนทับ (Overwrite) ไฟล์เดิมได้เลยทันที
